    ...
    def run(self):
        while True:
            ...
            try:
                ...
                print(f"Error: {e}")
                print([(k, v.shape) for k, v in img_array.items()])
                continue
...
class DualAirbotEnv(gym.Env):
    def __init__(
        ...
        self.action_space = gym.spaces.Box(
            np.concatenate(
                [self.env_left.action_space.low, self.env_right.action_space.low]
            ),
            np.concatenate(
                [self.env_left.action_space.high, self.env_right.action_space.high]
            ),
        )
    ...
    def step(self, action: np.ndarray) -> tuple:
        # print("DualAirbotEnv Step Action:", action)
        action_left = action[:len(action)//2]
        action_right = action[len(action)//2:]

        with ThreadPoolExecutor(max_workers=2) as ex:
            f_left = ex.submit(self.env_left.step, action_left)
            f_right = ex.submit(self.env_right.step, action_right)

            ob_left, reward_left, done_left, _, _ = f_left.result()
            ob_right, reward_right, done_right, _, _ = f_right.result()
    ...
    def reset(self, **kwargs):
        with ThreadPoolExecutor(max_workers=2) as ex:
            f_left = ex.submit(self.env_left.reset, **kwargs)
            f_right = ex.submit(self.env_right.reset, **kwargs)

            ob_left, _ = f_left.result()
            ob_right, _ = f_right.result()
